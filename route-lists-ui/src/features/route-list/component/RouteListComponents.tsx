...
export default observer(function RouteListComponents() {
    const { id } = useParams();
    ...
    const [items, setItems] = useState<RouteListComponent[]>([]);

    const { routeListComponentStore } = useStore();
    const { filter, saveScrollPosition, pageNumber, setPageNumber, setFilter } = routeListComponentStore;
    const [totalPages, setTotalPages] = useState(0);
    const filterRef = useRef<any>();

    useEffect(() => routeListComponentStore.restoreParameters(loadItems), []);

    useEffect(() => routeListComponentStore.restoreScrollPosition(items), [items])

    useEffect(() => routeListComponentStore.load(loadItems), [pageNumber, filter]);

    const loadItems = async () => {
        setLoading(true);

        agent.RouteListComponents.list(Number(id), pageNumber, filter)
            .then(result => {
    ...
    async function handleDelete() {
        ...
        agent.RouteListComponents.delete(itemForDeletingId)
            .then(() => loadItems())
            .finally(() => setLoading(false));
    }
            ...
            <Grid.Row>
                ...
                <Grid.Column width={12}>
                    <div style={{ overflowX: 'auto' }}>
                        {isLoggedIn && (
                            <Button as={NavLink}
                                to={`/createRouteListComponent/${id}`}
                                content='Добавить'
                                positive
                        ...
                        <Table celled size='small'>
                            <Table.Header>
                                <Table.Row>
                                    ...
                                    <Table.HeaderCell>Сопроводительный документ</Table.HeaderCell>
                            ...
                            <Table.Body>
                                {
                                    items.map(x => (
                                        <Table.Row key={x.id}>
                                            {isLoggedIn && (
                                                <Table.Cell>
                                                    ...
                                                    <div style={{ display: 'flex', flexDirection: 'row' }}>
                                                        <Button as={NavLink}
                                                            to={`/updateRouteListComponent/${x.id}`}
                                            ...
                                            <Table.Cell>{x.accompanyingDocument}</Table.Cell>
