...
import { CreateRouteListFramelessComponent } from '../../../app/models/CreateRouteListFramelessComponent';
import Spinner from '../../../app/components/Spinner';
import ValidationErrors from '../../../app/components/ValidationErrors';

export default observer(function CreateRouteListFramelessComponent() {
    ...
    const [data, setData] = useState<CreateRouteListFramelessComponent>({
        routeListId: 0,
        dateOfIssueForProduction: undefined,
        dateOfSealing: undefined,
        daysBeforeSealing: 0,
        ids: []
    });
    ...
    const loadItems = async () => {
        if (!loading) {
            ...
            agent.Products.getPurchasedProducts(pageNumber, filter)
                .then(result => {
    ...
    async function handleSubmit() {
        try {
            ...
            await agent.RouteListFramelessComponents.create(data);
                    ...
                    <div>
                        ...
                        <Form onSubmit={handleSubmit} autoComplete='off'>
                            ...
                            <Form.Field>
                                ...
                                <Form.Input name='dateOfIssueForProduction' type='date' value={data.dateOfIssueForProduction?.split('T')[0] ?? ''} onChange={handleInputChange} />
                            </Form.Field>

                            <Form.Field>
                                <label>Дата герметизации</label>
                                <Form.Input name='dateOfSealing' type='date' value={data.dateOfSealing ?? ''} onChange={handleInputChange} />
                            </Form.Field>

                            <Form.Field>
                                <label>Срок до герметизации</label>
                                <Form.Input name='daysBeforeSealing' type='number' value={data.daysBeforeSealing ?? ''} onChange={handleInputChange} />
                            </Form.Field>
