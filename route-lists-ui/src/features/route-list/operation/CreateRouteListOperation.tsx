...
import { CreateRouteListOperation } from '../../../app/models/CreateRouteListOperation';
import InfiniteScroll from 'react-infinite-scroller';
import Spinner from '../../../app/components/Spinner';
import ValidationErrors from '../../../app/components/ValidationErrors';

export default observer(function CreateRouteListOperation() {
    ...
    const [data, setData] = useState<CreateRouteListOperation>({
        department: 0,
        description: '',
        count: 0,
        type: '',
    ...
    useEffect(() => {
        handleCountAndNumber();
    }, []);

    async function handleCountAndNumber() {
        setLoading(true);

        try {
            let areThereOperations = await agent.RouteListOperations.areThereOperations(Number(id));
            let routeList = await agent.RouteLists.get(Number(id));

            if (!areThereOperations)
                setCount(routeList.productCount);

            setNumber(await agent.RouteListOperations.getNewNumber(Number(id)));
        } catch (error) {
            throw error;
        } finally {
            setLoading(false);
    ...
    async function handleSubmit() {
        try {
            ...
            data.count = count;
            data.number = number;
            await agent.RouteListOperations.create(data);
                ...
                <Grid.Column width={4}>
                    <div>
                        ...
                        <Form onSubmit={handleSubmit} autoComplete='off'>
                            ...
                            <Form.Field>
                                <label>Номер</label>
                                <Form.Input name='number' value={number} onChange={(e) => setNumber(e.target.value)} />
                            </Form.Field>
