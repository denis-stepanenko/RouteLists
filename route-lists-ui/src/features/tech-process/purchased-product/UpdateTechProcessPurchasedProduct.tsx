...
export default function UpdateTechProcessPurchasedProduct() {
    const { id } = useParams();
    ...
    const [item, setItem] = useState<TechProcessPurchasedProduct>({} as TechProcessPurchasedProduct);

    useEffect(() => {
        if (id) {
            setLoading(true);

            agent.TechProcessPurchasedProducts.get(Number(id))
                .then(x => setItem(x))
    ...
    async function handleSubmit() {
        try {
            setSubmitting(true);
            await agent.TechProcessPurchasedProducts.update(item);
